...
3. **两套皮肤**：同一变量名提供 `default`（亮色）与 `dark`（护眼暗色）两套取值。
...
## 5. 扩展色（Extension，DESIGN.md 未定义，综合各端现状补齐）
...
**扩展色暗色取值**（§5 扩展色在 dark 皮肤下的取值，default 值见 §6）：
